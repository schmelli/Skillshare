...
**Domain:** Self-hosted AI agent skill registry/hub (backend API + web dashboard + CLI + MCP server)
**Researched:** 2026-07-19
**Confidence:** MEDIUM (all findings from cross-checked web search; no official Context7/docs MCP available in this environment — versions verified against npm/official release pages via search, not training data)
...
```bash
# Monorepo scaffold
pnpm dlx create-turbo@latest skillshare
cd skillshare

# Backend (apps/api)
pnpm --filter api add @nestjs/core @nestjs/common @nestjs/platform-fastify \
  @nestjs/swagger prisma @prisma/client better-auth @better-auth/api-key \
  gray-matter zod pg-boss @modelcontextprotocol/sdk

# Frontend (apps/web)
pnpm --filter web add react react-dom @tanstack/react-query @mdxeditor/editor \
  react-diff-viewer-continued tailwindcss

# CLI (apps/cli)
pnpm --filter cli add commander zod

# Dev dependencies (root)
pnpm add -D -w typescript vitest @playwright/test eslint prettier turbo
```
...
**If the team has stronger Python/data-science leanings and doesn't need a unified TS monorepo:**
...
**If/when the project moves toward multi-tenant SaaS (the stated "cloud-ready" future phase):**
...
---
*Stack research for: Self-hosted AI agent skill registry/hub platform*
*Researched: 2026-07-19*
